...
def p_Instrucao3(t): "Instrucao : Cond"                             ; t[0] = f'{t[1]}'
def p_Instrucao4(t): "Instrucao : Exp"                              ; t[0] = f'{t[1]}'
def p_Instrucao5(t): "Instrucao : Funcoes"                          ; t[0] = f'{t[1]}'
...
def p_Print3(t): "Print : KEY Exp"                                  ; t[0] = f'read'
def p_Print4(t): "Print : SPACE"                                    ; t[0] = f'pushs " "\nwrites'
def p_Print5(t): "Print : INT SPACES"                               ; t[0] = f' ' * t[1]
def p_Print6(t): "Print : PSTRING "                                 ; t[0] = f'pushs "{t[1][3:]}\nwrites'
def p_Print7(t): "Print : EMIT"                                     ; t[0] = f'writechr'
def p_Print8(t): "Print : CR"                                       ; t[0] = f'writeln'
...
def p_Termoi3(t): "Termoi : DUP"                                    ; t[0] = f'dup 1'
def p_Termoi4(t): "Termoi : DUP Lsinais"                            ; t[0] = f'dup 1\n{t[3]}'
def p_Termoi5(t): "termoi : 2DUP"                                    ; t[0] = f'pushsp\nload-1\npushsp\nload-1'

def p_cond1(t): "Cond : Exp IF prog ELSE prog THEN "                ; t[0] = f'{t[1]}\njz else{get_label_cond(t[2])}\n{t[3]}\njump endif{get_label_cond(t[6])}\nelse{get_label_cond(t[6])}:\n{t[5]}\nendif{get_label_cond(t[6])}:'
def p_cond3(t): "Cond : Exp IF prog THEN"                           ; t[0] = f'{t[1]}\njz else{get_label_cond(t[2])}\n{t[3]}\njump endif{get_label_cond(t[4])}\nelse{get_label_cond(t[4])}:\nendif{get_label_cond(t[4])}:'
...
def p_Sinal10(t): "Sinal : '<' '>'"                                  ; t[0] = f'not\nequal'

def p_Funcoes1(t): "Funcoes : ':' NOME Definicao ';'"               ; t[0] = f''; define_funcao(t[2], t[3])
def p_Funcoes2(t): "Funcoes : NOME"                                 ; t[0] = f'pusha {t[1]}\ncall'; funcs.get(t[1], 0); 
def p_Funcoes3(t): "Funcoes : Exp NOME"                             ; t[0] = f'{t[1]}\npusha {t[2]}\ncall'; funcs.get(t[2], 0); 

def p_Definicao1(t): "Definicao : Conteudo"                         ; t[0] = f'{t[1]}'
def p_Definicao2(t): "Definicao : Definicao Conteudo"               ; t[0] = f'{t[1]}\n{t[2]}'

def p_Conteudo1(t): "Conteudo : Lsinais"                            ; t[0] = f'{t[1]}'
def p_Couteudo2(t): "Conteudo : Exp "                               ; t[0] = f'{t[1]}'
...
#def p_Conteudo6(t): "Conteudo : Ciclo"                              ; t[0] = f'{t[1]}'

#ciclos estao mal
def p_Ciclo1(t) : "Ciclo : Lints DO '{' prog '}' LOOP"            ; t[0] = f'while{get_label_loop(t[3])}:\npushi {t[1]}\npushi {t[2]}\npushn 0\nsup\njz endwhile{get_label_loop(t[4])}\n{t[5]}\n pushg 0\npushi 1\nadd\nstoreg 0\njump while{get_label_loop(t[4])}\nendwhile{get_label_loop(t[4])}:'
def p_Ciclo2(t) : "Ciclo : Lints DO Instrucao LOOP"               ; t[0] = f'while{get_label_loop(t[3])}:\npushi {t[1]}\npushi {t[2]}\npushn 0\nsup\njz endwhile{get_label_loop(t[4])}\n{t[4]}\n pushg 0\npushi 1\nadd\nstoreg 0\njump while{get_label_loop(t[4])}\nendwhile{get_label_loop(t[4])}:'
